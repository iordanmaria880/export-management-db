...
SELECT * FROM iordan_produse_bestselling;

UPDATE iordan_comenzi
SET stare_comanda = 'expediata'
WHERE stare_comanda = 'confirmata';
SELECT * FROM iordan_comenzi;

SELECT * FROM iordan_produse WHERE discount = 2;

UPDATE iordan_produse
SET discount=(SELECT discount FROM iordan_produse WHERE id_produs = 3)
WHERE discount = 2;

SELECT * FROM iordan_produse WHERE id_produs IN (4,8,11,15,22,28);

SELECT * FROM iordan_produse WHERE denumire LIKE INITCAP('Tastatura%') AND stoc > 10000;

UPDATE iordan_produse
SET (pret, discount) = (SELECT pret, discount FROM iordan_produse WHERE id_produs = 57)
WHERE denumire LIKE INITCAP('Tastatura%') AND stoc > 10000;

DELETE FROM iordan_produse_bestselling
WHERE categorie IN ('periferice', 'componente PC');
SELECT * FROM iordan_produse_bestselling;

SELECT * FROM iordan_documente WHERE EXTRACT(YEAR FROM data) <2015;
DELETE FROM iordan_documente
WHERE EXTRACT(YEAR FROM data) < 2015;

SELECT * FROM iordan_fosti_parteneri WHERE data_ultimei_comenzi < TO_DATE ('01-01-2005','DD-MM-YYYY');
DELETE FROM iordan_fosti_parteneri
WHERE data_ultimei_comenzi < TO_DATE ('01-01-2005','DD-MM-YYYY');

SELECT * FROM iordan_documente;
DELETE FROM iordan_documente
WHERE tip_document = 'factura';
ROLLBACK;

DELETE FROM iordan_sedii
WHERE statut IN (SELECT statut FROM iordan_sedii WHERE id_sediu = 30);

SELECT * FROM iordan_produse_bestselling WHERE stoc BETWEEN 2000 AND 3000;
UPDATE iordan_produse_bestselling
SET pret = pret * 1.1
WHERE stoc BETWEEN 2000 AND 3000;

ALTER TABLE iordan_regiuni
DISABLE CONSTRAINT check_denumire;

SELECT * FROM iordan_regiuni;
UPDATE iordan_regiuni
SET denumire = UPPER(denumire);
SELECT * FROM iordan_regiuni;

/* interogari de tabele */

SELECT * FROM iordan_comenzi
WHERE id_partener = 4000
ORDER BY id_comanda;

SELECT * FROM iordan_parteneri_comerciali
WHERE LOWER(email) like '%.com' AND ID_tara = 10;

SELECT * FROM iordan_transport
WHERE LOWER(modalitate) != 'rutier' AND data_expedierii > TO_DATE('01-01-2020','DD-MM-YYYY')
ORDER BY data_expedierii DESC;

SELECT p.id_partener, p.nume, p.oras, c.id_comanda, c.data_comenzii
FROM iordan_parteneri_comerciali p, iordan_comenzi c
WHERE p.id_partener = c.id_partener;

SELECT e.id_export, s.id_sediu, s.oras, e.pret, e.cantitate, (e.pret * e.cantitate) valoarea_exportului
FROM iordan_exporturi e, iordan_sedii s
WHERE e.id_sediu = s.id_sediu AND (e.pret * e.cantitate) < ANY (SELECT (pret * cantitate) valoarea_exportului FROM iordan_exporturi WHERE id_sediu = 20)
AND e.id_sediu != 20
ORDER BY e.id_export;

SELECT ID_produs, denumire, categorie, pret
FROM iordan_produse
WHERE pret >= 70 AND pret <= 1000 AND categorie LIKE '%laptop%';

SELECT * FROM iordan_parteneri_comerciali
WHERE oras IN (SELECT oras FROM iordan_parteneri_comerciali WHERE nume = 'Teknosa')
ORDER BY id_partener;

SELECT c.id_comanda, e.pret, e.cantitate, (e.pret * e.cantitate) valoarea_totala, p.discount
FROM iordan_comenzi c, iordan_exporturi e, iordan_produse p
WHERE c.id_comanda = e.id_comanda AND p.id_produs = e.id_produs
AND discount IS NULL AND cantitate IN (4000);

SELECT p.id_partener, p.nume, c.id_comanda
FROM iordan_parteneri_comerciali p, iordan_comenzi c
WHERE p.id_partener = c.id_partener (+);

SELECT p.id_produs, e.id_comanda, p.categorie, p.pret, p.discount
FROM iordan_produse p, iordan_exporturi e
WHERE p.id_produs = e.id_produs (+)
AND p.discount < 10 AND p.pret BETWEEN 1000 AND 5000 AND p.categorie IN ('telefoane', 'laptopuri');

SELECT 'Mai sunt disponibile ' || stoc || ' unitati din produsul ' || INITCAP(denumire) || ', din categoria ' || UPPER(categorie)
FROM iordan_produse;

SELECT c.id_comanda, c.data_comenzii, t.data_expedierii, ROUND((t.data_expedierii-c.data_comenzii)) zile, t.modalitate, ROUND(MONTHS_BETWEEN(SYSDATE, t.data_expedierii))|| ' luni intre data expedierii si data curenta', ADD_MONTHS(t.data_expedierii, 3), NEXT_DAY(c.data_comenzii, 'MONDAY'), LAST_DAY(c.data_comenzii)
FROM iordan_comenzi c, iordan_transport t
WHERE c.id_comanda = t.id_comanda;

SELECT id_comanda, data_comenzii, ROUND(data_comenzii, 'MONTH'), TRUNC(data_comenzii, 'MONTH')
FROM iordan_comenzi
WHERE EXTRACT(YEAR FROM data_comenzii) IN (2020,2021,2022);

SELECT id_transport, data_expedierii, TO_CHAR(data_expedierii, 'DD.MM.YYYY') data_expedierii_comenzii
FROM iordan_transport
WHERE SUBSTR(data_expedierii,1,1) = '2';

SELECT id_produs,denumire ,categorie, pret, discount, (pret - (pret * NVL(discount,0)/100)) pret_dupa_reducere
FROM iordan_produse
WHERE SUBSTR(categorie,1,9)='accesorii';

SELECT id_produs, NVL2(discount, ' produs redus', 'produs neredus')
FROM iordan_produse;

SELECT COUNT(id_export) nr_exporturi, ROUND(MIN(pret * cantitate)) valoare_minima, ROUND(MAX(pret * cantitate)) valoare_maxima, ROUND(AVG(pret * cantitate)) valoare_medie
FROM iordan_exporturi;

SELECT MIN(data_comenzii) prima_comanda, MAX(data_comenzii) ultima_comanda, COUNT(id_comanda) nr_comenzi
FROM iordan_comenzi;

SELECT modalitate, ROUND(AVG(cost_total)) cost_mediu
FROM iordan_transport
GROUP BY modalitate
ORDER BY cost_mediu;
...
FROM iordan_transport
GROUP BY modalitate
HAVING AVG(durata) > 20;

SELECT t.nume, COUNT(*) nr_comenzi
FROM iordan_tari t, iordan_parteneri_comerciali p, iordan_comenzi c
WHERE t.id_tara=p.id_tara AND p.id_partener=c.id_partener
GROUP BY t.nume
HAVING COUNT(*) = 2;

SELECT r.denumire, MIN(p.pret) pret_minim, MAX(p.pret) pret_maxim
FROM iordan_regiuni r, iordan_tari t, iordan_parteneri_comerciali pc, iordan_comenzi c, iordan_exporturi e, iordan_produse p
WHERE r.id_regiune=t.id_regiune AND t.id_tara=pc.id_tara AND pc.id_partener=c.id_partener AND c.id_comanda=e.id_comanda AND e.id_produs = p.id_produs
GROUP BY r.denumire
ORDER BY denumire;

SELECT id_export, cantitate, (pret * cantitate) valoare_totala,ROUND(((pret*cantitate)-((pret*cantitate) *
CASE WHEN cantitate < 5000 THEN 0.05
WHEN cantitate BETWEEN 5000 AND 10000 THEN 0.1
WHEN cantitate > 10000 THEN 0.15
ELSE 0 END))) valoare_redusa
FROM iordan_exporturi;

SELECT c.id_comanda, t.modalitate, DECODE(LOWER(t.modalitate), 'rutier', 'de lunga durata', 'aerian', 'de scurta durata', 'maritim', 'de durata medie', null) tip_livrare
FROM iordan_comenzi c, iordan_transport t
WHERE c.id_comanda = t.id_comanda;

SELECT * FROM iordan_produse 
WHERE pret BETWEEN 1000 AND 3000
MINUS
SELECT * FROM iordan_produse 
WHERE categorie LIKE '_o%';

SELECT p.nume, COUNT(c.id_comanda) nr_comenzi
FROM iordan_parteneri_comerciali p, iordan_comenzi c
WHERE p.id_partener = c.id_partener
GROUP BY p.nume
MINUS
SELECT p.nume, COUNT(c.id_comanda) nr_comenzi
FROM iordan_parteneri_comerciali p, iordan_comenzi c
WHERE p.id_partener = c.id_partener AND SUBSTR(p.nume, 1,1)  IN ('M', 'R', 'D')
GROUP BY p.nume
ORDER BY nume DESC;

SELECT c.id_comanda, e.id_produs, e.cantitate, e.pret
FROM iordan_comenzi c, iordan_exporturi e
WHERE c.id_comanda = e.id_comanda
AND e.cantitate > 10000
UNION
SELECT c.id_comanda, e.id_produs, e.cantitate, e.pret
FROM iordan_comenzi c, iordan_exporturi e
WHERE c.id_comanda = e.id_comanda
...
SELECT p.nume, c.id_comanda, COUNT(c.id_comanda) nr_comenzi, r.denumire
FROM iordan_parteneri_comerciali p, iordan_comenzi c, iordan_tari t, iordan_regiuni r
WHERE p.id_partener = c.id_partener (+) AND p.id_tara = t.id_tara AND t.id_regiune = r.id_regiune
GROUP BY p.nume, c.id_comanda, r.denumire
MINUS
SELECT p.nume, c.id_comanda, COUNT(c.id_comanda) nr_comenzi, r.denumire
FROM iordan_parteneri_comerciali p, iordan_comenzi c,  iordan_tari t, iordan_regiuni r
WHERE p.id_partener = c.id_partener (+)  AND p.id_tara = t.id_tara AND t.id_regiune = r.id_regiune
AND UPPER(denumire) = 'EUROPA'
GROUP BY p.nume, c.id_comanda, r.denumire
...
SELECT pf.id_punct, pf.oras, COUNT(e.id_export) nr_exporturi, pf.tip
FROM iordan_puncte_frontiera pf, iordan_exporturi e
WHERE pf.id_punct = e.id_punct
GROUP BY pf.id_punct, pf.oras, pf.tip
HAVING COUNT(e.id_export) <=1
INTERSECT
SELECT pf.id_punct, pf.oras, COUNT(e.id_export) nr_exporturi, pf.tip
FROM iordan_puncte_frontiera pf, iordan_exporturi e
WHERE pf.id_punct = e.id_punct AND pf.tip != 'terestru'
GROUP BY pf.id_punct, pf.oras, pf.tip

SELECT id_sediu, oras, id_sediu_parinte, LEVEL
FROM iordan_sedii
CONNECT BY PRIOR id_sediu = id_sediu_parinte
START WITH id_sediu = 10
ORDER BY LEVEL;

SELECT id_sediu, oras,level-1 nr_sedii_superioare,  SYS_CONNECT_BY_PATH(id_sediu,'/') id_sedii_superioare
FROM iordan_sedii
START WITH id_sediu = 10
CONNECT BY PRIOR id_sediu = id_sediu_parinte;
...
SELECT id_sediu, oras, id_sediu_parinte, level
FROM iordan_sedii
WHERE id_tara BETWEEN 0 AND 150
CONNECT BY PRIOR id_sediu = id_sediu_parinte
START WITH id_sediu = 10;

SELECT id_sediu, oras, id_sediu_parinte, level
FROM iordan_sedii
CONNECT BY id_sediu = PRIOR id_sediu_parinte
START WITH INITCAP(oras)='Bogota';

SELECT id_sediu, oras, id_sediu_parinte, level
FROM iordan_sedii
CONNECT BY PRIOR id_sediu = id_sediu_parinte
START WITH id_sediu = 20
ORDER BY level;
...
SELECT s.id_sediu, s.oras, s.id_sediu_parinte, r.denumire, level
FROM iordan_sedii s, iordan_regiuni r, iordan_tari t 
WHERE s.id_tara = t.id_tara AND t.id_regiune = r.id_regiune
AND UPPER(r.denumire) != 'EUROPA'
CONNECT BY PRIOR s.id_sediu = s.id_sediu_parinte
START WITH s.id_sediu = 10;

SELECT id_sediu, oras, level-1 nr_sedii_superioare, SYS_CONNECT_BY_PATH(oras, '/') oras_sediu_superior
FROM iordan_sedii
CONNECT BY PRIOR id_sediu = id_sediu_parinte
START WITH oras = 'Cape Town' OR oras = 'Berlin'
ORDER BY LEVEL;

CREATE VIEW PartenerCuComenzi AS
SELECT p.id_partener, p.nume, c.id_comanda, c.data_comenzii, COUNT(id_comanda) nr_comenzi
FROM iordan_parteneri_comerciali p, iordan_comenzi c
WHERE p.id_partener = c.id_partener
GROUP BY  p.id_partener, p.nume, c.id_comanda, c.data_comenzii;

SELECT * FROM PartenerCuComenzi;

CREATE VIEW ProduseReduse AS
SELECT id_produs, denumire, stoc, pret, (pret - (pret * discount/100)) pret_redus
FROM iordan_produse
WHERE categorie = 'telefoane' OR categorie = 'laptopuri'
WITH CHECK OPTION;

SELECT * FROM ProduseReduse;

UPDATE ProduseReduse
SET stoc = 5000
WHERE id_produs = 80;

SELECT * FROM ProduseReduse WHERE id_produs =80;

DROP VIEW PartenerCuComenzi;
...
INSERT INTO  iordan_produse_bestselling (id_produs, denumire, pret) VALUES (seq_produse.NEXTVAL, 'Telefon mobil Apple Iphone 11 Pro White', 3100);

ALTER SEQUENCE seq_produse
MAXVALUE 9;

INSERT INTO  iordan_produse_bestselling (id_produs, denumire, pret) VALUES (seq_produse.NEXTVAL, 'Laptop ASUS', 4000);
INSERT INTO  iordan_produse_bestselling (id_produs, denumire, pret) VALUES (seq_produse.NEXTVAL, 'Telefon mobil Samsung A20', '950');

DROP SEQUENCE seq_produse;

CREATE OR REPLACE SYNONYM iordan_bestsellers FOR iordan_produse_bestselling;

SELECT id_produs, denumire, categorie
FROM iordan_bestsellers
WHERE pret BETWEEN 500 AND 1000;

DROP SYNONYM iordan_bestsellers;

CREATE INDEX idx_categorie ON iordan_produse_bestselling(categorie);

ALTER INDEX idx_categorie RENAME TO idx_c;

SELECT * /*+ INDEX(iordan_produse_bestselling idx_c) */
FROM iordan_produse_bestselling 
WHERE categorie = 'telefoane';
